...
inline void Sleep(int seconds, EFI_RUNTIME_SERVICES* runtime_services){
    EFI_TIME current_time;
    runtime_services->GetTime(&current_time, NULL);
    int result_seconds = current_time.Second + seconds;


    while(current_time.Second < result_seconds){
        runtime_services->GetTime(&current_time, NULL);
    }
}
