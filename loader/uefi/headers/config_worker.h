...
inline void AnalyzeConfig(EFI_ALLOCATE_POOL allocate_protocol, EFI_COPY_MEM copy_mem_protocol){
    ...
    option_table['N' - ASCII_ALPHABET_OFFSET] = (void**)&boot_name;
    option_table['F' - ASCII_ALPHABET_OFFSET] = (void**)&file_path;
    option_table['A' - ASCII_ALPHABET_OFFSET] = (void**)&autoboot;

    CHAR16 service_char = 0;
    void** current_parametr;
    ...
    for(int i = 0; i < config_size; i++){
        ...
        allocate_protocol(EfiLoaderData, (current_parametr_length + 1) * 2, current_parametr);
